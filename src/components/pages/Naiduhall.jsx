import React from 'react'
import LiveButton from '../atoms/LiveButton'
import GithubButton from '../atoms/GithubButton'
import { CircleDashed } from "lucide-react"
import Link from 'next/link'

const NaiduhallMetaData = {
  githubRepo: "https://github.com/Febiecode/naiduhall",
  live: "https://naiduhall.vercel.app/",
  images: [
    "/static/naiduhall/landing.png",
  ],
  title: "Naiduhall Dashboard",
  description: "The Naiduhall Dashboard provides an overview of project statuses with visual data representation and recent activities. It includes master management features for processes, machines, employees, lines, styles, and SAM entries, allowing for efficient additions and deletions.",
  keyFeatures: [
    "Developed a monitoring and management dashboard for Naiduhall.",
    "Overview of project status with total, in-progress, finished, and unfinished project cards, complemented by bar and bubble graphs, and a recent project table.",
    "Functionality to add and remove process entries.",
    "Capability to manage machine records through add and delete operations.",
    "Features for adding and deleting employee records.",
    "Management of line entries with options to add and delete."
  ]
}

const Naiduhall = () => {
  return (
    <div className='mx-auto max-w-3xl px-4 sm:px-9 xl:max-w-5xl xl:px-0 mt-20'>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 items-center justify-between'>
        <div>
          <h1 className='text-xl font-semibold sm:text-2xl md:text-3xl xl:text-3xl mt-5'>
            {NaiduhallMetaData.title}
          </h1>
        </div>
        <div className='flex justify-start lg:justify-end md:justify-end gap-5 mt-5'>
          <LiveButton link={NaiduhallMetaData.live} />
          <GithubButton link={NaiduhallMetaData.githubRepo} />
        </div>

      </div>

      <div className='my-5'>
        <div className='flex justify-center'>
          {NaiduhallMetaData.images.map((image, index)=> (
            <img src={image} alt="erpAnswerSnap" className='mt-10 rounded-xl border-2 border-[#57575787]' key={index}/>
          ))}
        </div>


        <div className='mt-10'>
          <p className='font-semibold'>{NaiduhallMetaData.des}</p>

          <div className='mt-10 space-y-5'>
            <h1 className='text-lg font-bold sm:text-md md:text-lg lg:text-xl'>Key Features</h1>
            <ul className='space-y-3 mt-5'>
              {NaiduhallMetaData.keyFeatures.map((point,index) => (
                <li className='flex items-center gap-3' key={index}>
                  <div className='w-5'>
                    <CircleDashed size={15} style={{ color: '#004AAD' }} />
                  </div>
                  <p>{point}</p>
                </li>
              ))}
            </ul>

            <div className=' dark:bg-[#2020238f] bg-Light p-5 rounded-md border border-1 border-[#57575787] flex flex-col md:flex-row lg:flex-row justify-center items-center'>
              <div>
                <h1 className='text-sm font-bold sm:text-sm'>Build with:</h1>
              </div>
              <div className='flex space-x-2 ms-2'>
                <Link href={"https://nextjs.org/"} target='blank'>
                  <span className='text-[12px] font-semibold flex  items-center '><img src={"/static/icons/nextjs.svg"} alt="next.js" width={20} height={20} /> &nbsp;Next.js</span>
                </Link>
                <Link href={"https://vercel.com/"} target='blank'>
                  <span className='text-[12px] font-semibold flex items-center '><img src={"/static/icons/vercel.svg"} alt="vercel" width={20} height={20} /> &nbsp;Vercel</span>
                </Link>
                <Link href={"https://tailwindcss.com/"} target='blank'>
                  <span className='text-[12px] font-semibold flex items-center '><img src={"/static/icons/tailwindcss.svg"} alt="tailwindcss" width={20} height={20} /> &nbsp;Tailwind CSS</span>
                </Link>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

export default Naiduhall